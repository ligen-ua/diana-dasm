...
void PlatformDeleteFile(const orthia::PlatformString_type & fullFileName)
{
...
void GetUtcTime(SYSTEMTIME * pTime)
{
    memset(pTime, 0, sizeof(*pTime));
    GetSystemTime(pTime);
}

bool ConvertFileTimeToSystemTime(long long fileTime, SYSTEMTIME * pTime)
{
    memset(pTime, 0, sizeof(*pTime));
    LARGE_INTEGER value;
    ...
    FILETIME fileTime2;
    fileTime2.dwHighDateTime = value.HighPart;
    fileTime2.dwLowDateTime = value.LowPart;

    return FileTimeToSystemTime(&fileTime2, pTime) != FALSE;
}
bool UtcTimeToLocal(const SYSTEMTIME& utcTime, SYSTEMTIME* pLocalTime)
{
    return SystemTimeToTzSpecificLocalTime(0, &utcTime, pLocalTime) != FALSE;
}
